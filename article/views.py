...
def article_list_view(request):
    ...
    all_articles = Article.objects.filter(is_published=True).select_related('category').prefetch_related('tags')
    ...
    if sort_order == 'oldest':
        ...
        all_articles = all_articles.order_by('-pub_date')  # od najnowszych (domyślnie)
...
def category_detail_view(request, category_id):
    ...
    articles_in_category = Article.objects.filter(category_id=category_id).order_by('-pub_date')
